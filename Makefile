...
manual-deploy-sudo:
	...
	kubectl patch -f k8s/app-store-service-k8s-deployment.yaml -p '{"spec": {"template": {"spec": { "containers": [{"name": "app-store-service-v2", "image": "089941056973.dkr.ecr.us-east-1.amazonaws.com/app-store-service-v2:$(UUID)"}] } } } }' --local -o yaml | kubectl apply -f -
